...
const w = loadPage(target, process.argv[3] || "");
setTimeout(() => {
  const btn = [...w.document.querySelectorAll("button")].find(b => b.textContent.includes("Фильтры"));
  console.log("Filter button text:", btn ? btn.textContent : "(not found)");
  btn.dispatchEvent(new w.MouseEvent("click", { bubbles: true }));
  setTimeout(() => {
    console.log("Filters panel visible after click:", w.document.body.textContent.includes("Используется") ? "PASS" : "FAIL");
    console.log("Body contains literal 'undefined':", w.document.body.textContent.includes("undefined") ? "FAIL (bug)" : "PASS (clean)");
  }, 50);
}, 150);
